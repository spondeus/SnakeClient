...
public class TutorialScreen implements Screen {
    ...
    private void setupUI(){
        ...
        // Create a table to hold all the UI elements
        Table table = new Table();
        table.setFillParent(true);
        table.defaults().pad(10f);

        // Create a label for the title
        Label titleLabel = new Label("How to Play", new Label.LabelStyle(new BitmapFont(), Color.WHITE));
        table.add(titleLabel).colspan(2).center().top();
        table.row();

        // Add the left side labels and descriptions
        Label moveLabel = new Label("Controls -", new Label.LabelStyle(new BitmapFont(), Color.WHITE));
        Label moveDescriptionLabel = new Label(
                "Press A to turn left \n Press D to turn right",new Label.LabelStyle(new BitmapFont(), Color.WHITE
        ));
        table.add(moveLabel).right();
        table.add(moveDescriptionLabel).left().expandX().fillX();
        table.row();

        // Add the pickup types and images
        Label pickupTypesLabel = new Label("Pickup types", new Label.LabelStyle(new BitmapFont(), Color.WHITE));
        table.add(pickupTypesLabel).colspan(2).center().top();
        table.row();

        // Create a horizontal group to hold the pickup images
        HorizontalGroup pickupImages = new HorizontalGroup();
        pickupImages.space(20f);

        // Add the pickup images to the horizontal group
        TextureRegionDrawable foodImage = new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("food.png"))));
        Image foodIcon = new Image(foodImage);
        foodIcon.setScale(0.5f);
        pickupImages.addActor(foodIcon);

        TextureRegionDrawable poisonImage = new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("poison.png"))));
        Image poisonIcon = new Image(poisonImage);
        poisonIcon.setScale(0.5f);
        pickupImages.addActor(poisonIcon);

        TextureRegionDrawable energyDrinkImage = new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("energydrink.png"))));
        Image energyDrinkIcon = new Image(energyDrinkImage);
        energyDrinkIcon.setScale(0.5f);
        pickupImages.addActor(energyDrinkIcon);

        TextureRegionDrawable spiderWebImage = new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("spiderweb.png"))));
        Image spiderWebIcon = new Image(spiderWebImage);
        spiderWebIcon.setScale(0.5f);
        pickupImages.addActor(spiderWebIcon);

        TextureRegionDrawable iceBlockImage = new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("iceblock.png"))));
        Image iceBlockIcon = new Image(iceBlockImage);
        iceBlockIcon.setScale(0.5f);
        pickupImages.addActor(iceBlockIcon);

        TextureRegionDrawable ghostImage = new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("ghost.png"))));
        Image ghostIcon = new Image(ghostImage);
        ghostIcon.setScale(0.5f);
        pickupImages.addActor(ghostIcon);

        // Add the horizontal group of pickup images to the table
        table.add(pickupImages).colspan(2).expandX().center().row();

        // Add pickup descriptions to the table
        ...
        // Create a vertical group to hold the pickup descriptions
        VerticalGroup pickupDescriptionsGroup = new VerticalGroup().space(10f);

        // Loop through the pickup types and add a label for each pickup description
        for (int i = 0; i < pickupTypes.length; i++) {
            Label pickupLabel = new Label(pickupTypes[i].name(), skin, "tutorial");
            Label descriptionLabel = new Label(pickupDescriptions[i], skin);

            // Add the pickup label and description to the vertical group
        ...
        table.add(pickupDescriptionsGroup).padTop(50f).left();

        // Create a new cell for the pickup images
        Cell<HorizontalGroup> pickupImagesCell = table.add(pickupImages);

        // Add a new column to the table for the pickup descriptions
        table.columnDefaults(0).padLeft(20).padRight(20);
    ...
    private String getPickupDescription(Type type) {
        switch (type){
            case FOOD: return "Increases snake's body length";
            case POISON: return "Decreases snake's body length";
            case DRINK: return "Speeds up snake movement";
            case WEB: return "Slows down the snake";
            case ICE: return "Freezes the snake for 5 seconds";
            case GHOST: return "Enables the snake to go through walls and other snakes";
            default: return "";
        }
    }
    ...
    private TextButton addButton(String name) {
        ...
        table.add(button).width(300).height(100).padBottom(5);
        table.bottom();
        return button;
    }
